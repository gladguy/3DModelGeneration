...
def getTotalPoints(base_path, m, maskright, maskleft, maskrightindex, maskleftindex, cameraWidth, cameraHeight):
    ...
    m, "m"
    print
    len(maskrightindex), len(rightcodmean), len(maskright)
    ...
    for ii in range(0, m - 1):
        if (maskleft[ii] and maskright[ii]):
            ...
            xyzdata, abdist = camxyzparam([matchpixels[0], matchpixels[1], matchpixels[2], matchpixels[3]], cameraWidth,
                                          cameraHeight)
            ...
            if ((np.linalg.norm(abdist) < 20) and (
                    xyzdata[2] > 400 and xyzdata[2] < 2000 and xyzdata[0] > -500 and xyzdata[0] < 500)):
                xyz[kk, 0] = -xyzdata[0]
                xyz[kk, 1] = -xyzdata[1]
                xyz[kk, 2] = -xyzdata[2]
                # color info from average of image from both cameras
                xyzcolor[kk, 0] = (camrcolor[int(matchpixels[1]), int(matchpixels[0]), 2] / 2 + camrcolol[
                    int(matchpixels[3]), int(matchpixels[2]), 2] / 2)
                xyzcolor[kk, 1] = (camrcolor[int(matchpixels[1]), int(matchpixels[0]), 1] / 2 + camrcolol[
                    int(matchpixels[3]), int(matchpixels[2]), 1] / 2)
                xyzcolor[kk, 2] = (camrcolor[int(matchpixels[1]), int(matchpixels[0]), 0] / 2 + camrcolol[
                    int(matchpixels[3]), int(matchpixels[2]), 0] / 2)
                kk = kk + 1
...
def saveAll(base_path, kk, xyz, xyzcolor):
    ...
    for ii in range(0, kk - 1):
        ff.write(
            str(xyz[ii, 0]) + " " + str(xyz[ii, 1]) + " " + str(xyz[ii, 2]) + " " + str(xyzcolor[ii, 0]) + " " + str(
                xyzcolor[ii, 1]) + " " + str(xyzcolor[ii, 2]) + "\n")
...
kk, xyz, xyzcolor = getTotalPoints(base_path, m, maskright, maskleft, maskrightindex, maskleftindex, cameraWidth,
                                   cameraHeight)
