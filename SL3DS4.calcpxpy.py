...
def createMask(threshold, firstImage):
    ...
    print(imgmask, "is imgmask", len(imgmask), "is length")
...
def getSortedUniqueColoc(horzlino, vertlino, Rcamcode, Lcamcode, imgmaskR, imgmaskL):
    ...
    np.savetxt(base_path + "4_leftcod", colocleft, fmt='%d', delimiter=', ', newline='\n')
    np.savetxt(base_path + "4_rightcod", colocright, fmt='%d', delimiter=', ', newline='\n')
    ...
    np.save(base_path + "4_colocleftsrt", colocleftsrt)
    newlistl = np.unique(leftsrtt)
    np.savetxt(base_path + "4_colocleftsrtuniq", newlistl, fmt='%d', delimiter=', ', newline='\n')
    np.save(base_path + "4_colocrightsrt", colocrightsrt)
    newlistr = np.unique(rightsrtt)
    np.savetxt(base_path + "4_colocrightsrtuniq", newlistr, fmt='%d', delimiter=', ', newline='\n')
...
def getMatchpixels(newlistr, newlistl, colocrightsrt, colocleftsrt, rightsrtt, leftsrtt):
    ...
    """
    np.savetxt(base_path + "4_colocuniq", matchpixels, fmt='%d', delimiter=',', newline='\n')
...
rightcamcode = np.load(base_path + "CAMR\\2_coloccod.npy")
leftcamcode = np.load(base_path + "CAML\\2_coloccod.npy")

# threshold
thresholdleft = np.load(base_path + "3_thresholdleft.npy")
thresholdright = np.load(base_path + "3_thresholdright.npy")
print(thresholdleft, thresholdright)
# firstImage
...
np.save(base_path + "4_imgmaskl", imgmaskleft)
imgmaskright = createMask(thresholdright, imgmaskrightf)
np.save(base_path + "4_imgmaskr", imgmaskright)
# visualizeImage(imgmaskleft, "imgmaskleft")
print("STEP 1: Mask done!")
(newlistr, newlistl,
 colocrightsrt, colocleftsrt,
 rightsrtt, leftsrtt) = getSortedUniqueColoc(horzlino, vertlino,
                                             rightcamcode, leftcamcode,
                                             imgmaskright, imgmaskleft)
print("STEP2: sorted and saved!")
getMatchpixels(newlistr, newlistl,
               colocrightsrt, colocleftsrt,
               rightsrtt, leftsrtt)
